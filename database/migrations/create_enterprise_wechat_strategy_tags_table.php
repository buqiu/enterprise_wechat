...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('strategy_tags', function (Blueprint $table) {
            ...
            $table->unique(['corp_id', 'group_id', 'tag_id'], DB::getTablePrefix().'corp_strategy_tag_unique');
            $table->index(['corp_id', 'group_name'], DB::getTablePrefix().'corp_group_name_index');
            $table->dateTime('created_at');
            $table->dateTime('updated_at');
        });
        DB::statement('ALTER TABLE `'.DB::getTablePrefix()."strategy_tags` comment '规则组标签表'");
    }
